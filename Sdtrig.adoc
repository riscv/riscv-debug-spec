...
[#norm:Sdtrig_match]#A trigger matches when the conditions that it specifies (e.g. a load
from a specific address) are met.# [#norm:Sdtrig_fire]#A trigger fires when a trigger that
...
|===
^| Value | Description
|0 | Raise a breakpoint exception. (Used when software wants to use the trigger module without an external debugger attached.) [#norm:Sdtrig_action0_xepc]#`xepc` must contain the virtual address of the next instruction that must be executed to preserve the program flow.#
...
***Link to the privileged spec table
...
[norm:Sdtrig_native_reentry]#Harts that support triggers with {mcontrol-action}=0 should implement one of the following two solutions to solve the problem of reentrancy:#
...
have hard-to-find bugs. [#norm:Sdtrig_caches_stores]#For the purposes of debug triggers, two classes
...
[#norm:Sdtrig_partial_exec_prevent]#Alternatively, it may state that partial execution is not allowed,
implying that a mid-execution trigger must prevent any architectural
state changes from occurring.#

[#norm:Sdtrig_resume]#Debuggers won't be aware if an instruction has been partially executed.
...
supported (which may simply be a disabled trigger). [#norm:Sdtrig_tdata_read_back]#This means that a
