...
optional.
|[[transfer]] `transfer`
|0 (disabled): Don’t do the operation specified by <<write,write>>. +
...
addresses equal physical) may optionally allow <<aamvirtual>> set to 1, which would produce the same result as that same abstract command with <<aamvirtual>> cleared.
...
(which contains the address used) by the number of bytes encoded in <<aamsize>>.
