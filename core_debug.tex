...
debugged runs in a less privileged mode. When a step is required, the OS or
debug stub writes \FcsrIcountCount=1, \FcsrIcountAction=0, \FcsrIcountM=0 before
returning control to the lower user program with an \tt{mret} instruction.

It also allows single step on tiny system which only supports M-Mode, but the
code becomes tricky to get right. To single step, the debug stub would execute
something like:
\begin{verbatim}
    ...
    ret                 /* Return to program being debugged. count decrements to 1 */
\end{verbatim}
