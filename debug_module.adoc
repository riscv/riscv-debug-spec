...
subordinate (see xref:implementations.adoc#tab:dmi_signals[]), or use a more
...
include::{debug-partials}abstract_commands.adoc[]
...
(see xref:Sdext.adoc#debugmode[]). If an exception is
...
image::{debug-images}abstract_commands.png[]
...
include::{debug-partials}dm_registers.adoc[]
