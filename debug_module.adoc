...
writing 1 to {dmstatus-ackhavereset} in {dm-dmcontrol}. The `havereset` bits might or might not be cleared when {dmcontrol-dmactive} is low.
...
select a hart by writing its index to {dm-dmcontrol,hartsel}. Hart indexes start at 0 and are
