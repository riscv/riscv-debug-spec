...
|WARL |preset
|[[dmcstwohgwrite,dmcstwohgwrite]] `hgwrite` 
...
[[haltsumtwo,haltsumtwo]] 
Each bit in this read-only register indicates whether
...
[[haltsumthree,haltsumthree]] 
Each bit in this read-only register indicates
...
[[sbcssbaccesseight,sbcssbaccesseight]]
[float="center",align="center",cols="<2,<3,^1,<1",options="header"]
|===
|Field |Description |Access |Reset
...
progress (while <<sbbusy>> is set). It remains set until it's explicitly cleared by the debugger. +
...
Writes to <<sbcs>> while <<sbbusy>> is high resulting in undefined behavior. A debugger must not write to <<sbcs>> until it reads <<sbbusy>> as 0.
|R |0
...
3 (64bit): 64-bit +
4 (128bit): 128-bit +
If <<sbaccess>> has an unsupported value when the DM starts a bus access, the access is not performed and <<sberror>> is set to 4.
|R/W |2
|[[sbcssbautoincrement,sbcssbautoincrement]] `sbautoincrement` 
|When 1, `sbaddress` is incremented by the access size (in bytes) selected in after every system bus access.
 |R/W |0
...
|R |Preset
|[[sbcssbaccessonetwentyeight,sbcssbaccessonetwentyeight]] `sbaccess128` 
|1 when 128-bit system bus accesses are supported.
|R |Preset
|[[sbcssbaccesssixtyfour,sbcssbaccessonetwentyeight]] `sbaccess64`
|1 when 64-bit system bus accesses are supported.
|R |Preset
|[[sbcssbaccessthirtytwo,sbcssbaccessthirtytwo]] `sbaccess32`
|1 when 32-bit system bus accesses are supported.
|R |Preset
|[[sbcssbaccesssixteen,sbcssbaccesssixteen]] `sbaccess16`
...
[[sbaddresszeroaddress,sbaddresszeroaddress]]
[%autowidth,float="center",align="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
...
[[sbaddressoneaddress,sbaddressoneaddress]]
[%autowidth,float="center,align="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
...
[sbaddresstwo,sbaddresstwo]] If <<sbasize>> is less than 65, then this register is not present.

When the system bus manager is busy, writes to this register will set <<sbbusyerror>> and don't do anything else.

*FIXME:Register diagram goes here.*

[[sbaddresstwoaddress,sbaddresstwoaddress]]
[%autowidth,float="center",align="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
|`address`
...
[[sbaddressthreeaddress,sbaddressthreeaddress]]
[%autowidth,float="center",align="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
|`address` 
...
Only <<sbdata0>> has this behavior. The other `sbdata` registers have no side
effects. On systems that have buses wider than 32 bits, a debugger
should access <<sbdata0>> after accessing the other `sbdata` registers.

*FIXME: Register diagram goes here.*

[[sbdatazerodata,sbdatazerodata]]
[%autowidth,align="center",float="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
|`data` |Accesses bits 31:0 of `sbdata`. 
|R/W |0
|===
...
[[sbdataonedata,sbdataonedata]]
[%autowidth,align="center",float="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
|`data` |Accesses bits 63:32 of `sbdata` (if the system bus is that wide).
|R/W |0
|===
...
[[sbdatatwodata,sbdatatwodata]]
[%autowidth,float="center",align="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
|`data` |Accesses bits 95:64 of `sbdata` (if the system bus is that wide). 
|R/W |0
|===

==== System Bus Data 127:96 (`sbdata3`, at 0x3f)

[[sbdatathree,sbdatathree]]
This register only exists if <<sbaccess128>> is 1.
...
[[sbdatathreedata,sbdatathreedata]]
[%autowidth,float="center",align="center",cols="<,<,^,<",options="header"]
|===
|Field |Description |Access |Reset
|`data` |Accesses bits 127:96 of `sbdata` (if the system bus is that wide).
|R/W |0
|===
...
The optional through registers may be used for non-standard features. Future versions of the debug spec will not use these addresses.
