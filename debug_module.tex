...
To abort an abstract command, the debugger first resets the hart (using
\Fhartreset or \Fndmreset), and then the Debug Module (using \Fdmactive).

If a hart becomes unavailable while executing an abstract command then the
abstract command may be terminated, with \Fbusy going low, and \Fcmderr being
set to 4 (halt/resume). Alternatively, the command could just appear to be hung.
