...
Writing \Fhaltreq has no effect on a hart which is already halted.
To resume, the debugger selects the hart with \Fhartsel,
sets \Fresumereq, then waits for \Fallresumed to indicate the
hart is running.

\begin{commentary}
  A debugger should use \Fallresumed, not \Fallrunning
  or \Fallhalted,
