...
The hart array mask register will usually contain exactly one bit per hart
that's debuggable. It is permitted to be smaller (in which case only the harts
with lower indexes can be accessed simultaneously) or larger (in which case
setting the higher bits does not select additional harts).
