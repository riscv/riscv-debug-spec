...
reset at power-up, while
\FdmDmcontrolDmactive in \RdmDmcontrol is 0, or when the not recommended global
reset signal is implemented and asserted.
...
A debugger must only use \FdmDmcontrolDmactive to reset the debug module.  If the
debug module is reset by any other means when \FdmDmcontrolDmactive is 1, the
behavior of the debug module is undefined.
