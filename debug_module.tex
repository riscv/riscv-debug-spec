...
parts of the system except for the Debug Module and Debug Transport Modules.
...
clears it. The actual reset may start as soon as the bit is asserted, but is allowed
to start a long time after the bit is deasserted. While the reset is
...
immediately enter Debug Mode. Otherwise it will execute normally.

*** TODO *** How does a debugger know that the reset has started and they can
execute abstract commands during the reset? Having the debugger halt all harts
so it can tell whether they're running doesn't really work, because below we
say that hart state should be maintained? Why do we say that? 2017 change by
Megan.
*** /TODO ***
