...
When harts are reset, they should set a sticky {\tt hasreset} state bit.
The {\tt hasreset} state bits can be read for selected harts in the \Fanyhasreset
and \Fallhasreset in \Rdmstatus. This bit should be set whether the
hart reset under the direction of the debugger or not. It can be cleared
by writes to \Fhasresetack in \Rdmcontrol.
