...
if there are other harts with higher indexes than this one. The most common
example of this state is when a hart is held in reset. Other examples are a
hart that is temporarily powered down, or one that might be hot-swapped in
later.
...
Harts are running when they are executing normally, as if was no debugger was
