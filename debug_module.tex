...
If a hart has a cache, the data in the cache may not be consistent with the
memory as accessed directly through the system bus. It is up to the debugger to
have each hart write back its caches before performing a system bus read, and
to invalidate the caches after a system bus write. The simplest way to do this
is by executing cache control instructions using the Program Buffer. (Currently
there is no standard for those, so they will have to be target-specific.) If
there is no Program Buffer, the system must handle cache coherency itself.
