...
operations.  The low addresses of the address space are
...
The DM keeps track of which harts are selected. The debugger can add or remove
harts to or from the set of selected harts by writing to \Faggregate and then
\Rhartaggzero through \Rhartaggthree. A debugger will spend much of its time
interacting with one hart, but selecting multiple harts is also supported.

When multiple harts are selected, \Fhaltreq, \Fresumereq, and \Fhartreset apply
to all of them. If the DM supports it, abstract commands can also apply to
multiple harts. Operations that apply to multiple harts should apply
simultaneously if practical, but that is not required.

Hart indexes start at 0 and are contiguous until the final index.

To enumerate all the harts, a debugger must first determine {\tt AGGREGATELEN}
by writing all ones to \Faggregate (assuming the maximum size) and reading back
...
highest index (depending on {\tt AGGREGATELEN}) is reached. Alternatively it
can perform a binary search to find the lowest index that is non-existent.
