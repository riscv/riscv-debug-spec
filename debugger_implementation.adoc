...
the JTAG DTM described in xref:dtm.adoc#sec:jtagdtm[]. All these
...
For each bit set there, it writes xref:debug_module.adoc#dm-dmcontrol[hartsel], and checks {dm-haltsum2}. This process repeats
...
xref:Sdtrig.adoc#nativetrigger[], then a carefully written trap handler can
