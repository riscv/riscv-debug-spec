...
\section{Debug Module Interface Signals}

As stated in section \ref{dmi} the details of the DMI is left to the system designer.
...
which is the implementation used in the open-source rocket-chip RISC-V core:
https://github.com/sifive/rocket-chip/blob/master/src/main/scala/devices/debug/Debug.scala#L170
...
\begin{table}[ht]\centering
	\begin{tabular}[h]{l l l}
		...
		\hline
		REQ\_VALID 			& DTM		& Indicates that the a valid request is pending \\
		\hline
		REQ\_READY 			& DM		& Indicates that the DM is able to process a request \\
		\hline
		REQ\_ADDRESS		& DTM		& Requested address \\
		\hline
		REQ\_DATA			& DTM		& Requested data \\
		\hline
		REQ\_OP 			& DTM		& Same meaning as the \FdtmDmiOp field\\
		\hline
		RSP\_VALID 			& DM		& Indicates that the a valid respond is pending \\
		\hline
		RSP\_READY			& DTM		& Indicates that the DTM is able to process a respond \\
		\hline
		RSP\_DATA			& DM		& Response data \\
		\hline
		RSP\_OP				& DM		& Same meaning as the \FdtmDmiOp field\\
		\hline
	\end{tabular}
