...
In order for the DTM to perform a read it has to wait for REQ\_READY to be high.
Is this the case REQ\_OP can be set to 1 to indicate a read action and REQ\_ADDRESS to the required address.
Finally REQ\_VALID is set high, indicating to the DM that a valid request is pending.

The DM can respond to a request from the DTM when RSP\_READY is high.
The status of the respond is indicated with the RSP\_OP signal (See \FdtmDmiOp) and potential respond data with RSP\_DATA.
