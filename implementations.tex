...
Before entering the park loop, the hart will typically execute a {\tt fence} instruction, then write to the Debug Module indicating that the abstract command has finished.  The {\tt fence} ensures that any effects from the abstract command take effect before the DM returns \FdmAbstractcsBusy to 0.
