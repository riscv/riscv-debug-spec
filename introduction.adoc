include::{debug-partials}register-attrs.adoc[]
...
*DM*:: Debug Module (see xref:debug_module.adoc#dm[]).

*DMI*:: Debug Module Interface (see xref:debug_module.adoc#dmi[]).

*DR*:: JTAG Data Register.

*DTM*:: Debug Transport Module (see xref:dtm.adoc#dtm[]).
...
*Minimal RISC-V Debug Specification*:: A subset of the full Debug Specification that allows for very small implementations. See xref:debug_module.adoc#dm[].
...
*SBA*:: System Bus Access (see xref:debug_module.adoc#systembusaccess[]).
...
*TM*:: Trigger Module (see xref:Sdtrig.adoc#trigger[]).
...
. Resume ack is set after resume, in xref:debug_module.adoc#runcontrol[].
...
. All Trigger Module registers (xref:Sdtrig.adoc#tab:trigger[]) are optional. https://github.com/riscv/riscv-debug-spec/pull/431[#431]
...
. Specify trigger priorities in xref:Sdtrig.adoc#tab:priority[] relative to exceptions.
...
. Change quick access exceptions to halt the target in xref:debug_module.adoc#ac-quickaccess[].
...
. Solutions to deal with reentrancy in xref:Sdtrig.adoc#nativetrigger[] prevent triggers from
...
. Add halt groups and external triggers in xref:debug_module.adoc#hrgroups[].
...
. Add resume groups in xref:debug_module.adoc#hrgroups[].
...
// The index is generated only by the PDF build; Antora has no index page.
ifdef::backend-pdf[]
They are also listed in the <<index>>.
endif::[]

include::{debug-partials}sample_registers.adoc[]
