...
// Path roots for the register partials and figures shared with the Antora build.
// Antora overrides these in antora.yml with the equivalent resource IDs.
:debug-partials: modules/ROOT/partials/
:debug-images: modules/ROOT/images/
...
// Matches the antora.yml definition; without it the PDF printed a literal {dmcontrol}.
:dmcontrol: `dmcontrol`
