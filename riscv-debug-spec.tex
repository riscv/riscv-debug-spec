...
Program Buffer code) the hart jumps back to its dm\_park loop. If an exception
is encountered, the hart jumps to its debug exception address, which in turn
contains a jump back to its dm\_park loop. The DM infers from the fetch at the
dm\_park address that there was an exception, and sets \Fcmderr appropriately.
...
\begin{tabulary}{\textwidth}{|r|r|r|L|}
    ...
    Write & \Ribufone & {\tt ebreak} & \\
    \hline
    Write & \Rcommand & \Fexecb, 0x1008 & Execute program buffer, then read \Szero \\
    \hline
