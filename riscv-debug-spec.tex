...
There are two ways to access registers. The first is to use the Access Register
abstract command. This is the only way to access GPRs. Accessing other
registers is also possible using the Program Buffer. In that case the debugger
writes a simple program that transfers the value between a register it wants to
access and a GPR.

For example to read \Rmstatus, it writes the following program to the
Instruction Buffer:
\begin{minted}{gas}
        csrr    s0, MSTATUS
        ebreak
...
Next it executes the abstract command that causes this program to be executed
(0x21000). When \Fbusy in \Rabstractcs goes low, now the debugger executes the
command to read \Szero (TODO).

To write to eg. \Fone, the following program is written:
\begin{minted}{gas}
        fmv.x.s f1, s0
        ebreak
\end{minted}

The desired value is written to \Rdatazero, and then the abstract command to
write \Szero (TODO) and execute the program is executed. Once \Fbusy in
\Rabstractcs the write is complete.
