...
must implement one of the following two solutions to solve the problem of
reentrancy:}
\item The hardware must prevent triggers with \FcsrMcontrolSixAction=0 from
firing while in M-mode and while \FcsrMstatusMie in \Rmstatus is 0.  If
\Rmedeleg[3]=1 then it must prevent triggers with \FcsrMcontrolSixAction=0
from firing while in S-mode and while \FcsrSstatusSie in \Rsstatus is 0.
If \Rmedeleg[3]=1 and \Rhedeleg[3]=1 then it must prevent triggers with
