...
will be set using \Fu or \Fs, and when firing they can cause a debug exception
...
to. When such a trigger causes a debug exception while already in a trap
...
for native debugging even M mode code.

The simple solution is to have the hardware prevent triggers with action=0 from
firing while in M mode and while interrupts are disabled. Its limitation is
...
A user setting M mode triggers that cause debug exceptions will have to be
