...
\section{Self-hosted Triggers}
\label{sec:shtrigger}

Triggers can be used for self-hosted debugging when \FcsrMcontrolSixAction=0.
...
\begin{steps}{If a hart supports a configuration such that any of the cases
above may occur, one of the following two solutions must be implemented to
solve this problem:}
\item The hardware must prevent triggers with action=0 from firing while in
M-mode and while \FcsrMstatusMie in \Rmstatus is 0.  If \Rmedeleg[3]=1 then
it must prevent triggers with action=0 from firing while in S-mode and while
\FcsrSstatusSie in \Rsstatus is 0.  If \Rmedeleg[3]=1 and \Rhedeleg[3]=1 then
it must prevent triggers with action=0 from firing while in VS-mode and while
\FcsrSstatusSie in \Rvsstatus is 0.
\item \FcsrTcontrolMte and \FcsrTcontrolMpte in \RcsrTcontrol must be
implemented.  \Rmedeleg[3] must be hard-wired to 0.
\end{steps}
